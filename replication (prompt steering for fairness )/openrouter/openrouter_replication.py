...
# two new models, complementing the gpt-5.5 / gpt-3.5-turbo results:
# a reasoning model (does it also override the incentive, like gpt-5.5?)
# and a non-reasoning open-weights model (does it behave like gpt-3.5-turbo?).
# Any OpenRouter model slug works here.
MODELS = [
...
# reduced job set (10 jobs instead of 20) so that 20 rounds keep the
# "each job appears twice" structure of the original design
JOB_SECTORS = {
    "HC_LW": ["Lawyers", "Bankers", "Managers"],
    "HC_HW": ["Doctors", "Teachers", "Professors"],
    "LC_HW": ["Receptionists", "Waiters"],
    "LC_LW": ["Janitors", "Cashiers"],
}

JOB_TO_SECTOR = {} # DICTIONARY OF THE FORM {job: sector}, i.e {Lawyers: HC_LW, Bankers: HC_LW, etc.}
...
def make_job_sequence(rng: random.Random) -> List[str]: # list of 20 positions so that each job appears twice in random order
    jobs = ALL_JOBS * 2
...
def call_model(model: str, messages: List[Dict[str, str]]) -> str:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # OpenRouter can return a payload with no choices (provider error); retry it
            if not response.choices:
                raise EmptyModelResponse("response contained no choices")

            message = response.choices[0].message
            content = message.content

            # some providers leave content empty and put everything in the reasoning field
            ...
            return content
        except (RateLimitError, APIConnectionError, APITimeoutError, EmptyModelResponse) as e:
            if attempt == MAX_RETRIES:
                raise
            # jitter spreads out retries so parallel workers don't all retry at once
            wait = backoff + random.uniform(0, backoff / 2)
            print(f"  {model}: transient error ({type(e).__name__}), retry {attempt}/{MAX_RETRIES} in {wait:.1f}s")
            time.sleep(wait)
...
def main():
    ...
    partial_path = unique_output_path("openrouter_baseline_results_partial.csv")
    ...
    results_path = unique_output_path("openrouter_baseline_results.csv")
    ...
    summary_path = unique_output_path("openrouter_baseline_summary.csv")
